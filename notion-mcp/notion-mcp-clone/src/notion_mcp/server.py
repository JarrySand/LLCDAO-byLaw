...
from .models.notion import Database, Page, SearchResults, Block, Comment, CommentList
...
async def list_tools() -> List[Tool]:
    ...
    return [
        ...
        Tool(
            name="get_comments",
            description="Get comments for a specific Notion page or block",
            inputSchema={
                "type": "object",
                "properties": {
                    "block_id": {
                        "type": "string",
                        "description": "ID of the page or block to get comments for"
                    },
                    "start_cursor": {
                        "type": "string",
                        "description": "Cursor for pagination"
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "content": {
                        "type": "string",
                        "description": "Content of the comment (plain text)"
                    }
                },
        ...
        Tool(
            name="get_comments_with_context",
            description="Get comments for a Notion page with detailed context about what content was commented on",
            inputSchema={
                "type": "object",
                "properties": {
                    "block_id": {
                    ...
                    "include_context": {
                        "type": "boolean",
                        "description": "Whether to include detailed context about commented content",
                        "default": True
                    },
                    "start_cursor": {
                        "type": "string",
...
async def call_tool(name: str, arguments: Any) -> Sequence[Union[TextContent, EmbeddedResource]]:
    ...
    try:
        if name == "list_databases":
            ...
            comments = await notion_client.get_comments(
                block_id=block_id,
                start_cursor=arguments.get("start_cursor"),
                page_size=arguments.get("page_size", 100)
            )
            return [
                TextContent(
                    type="text",
                    text=comments.model_dump_json(indent=2)
                )
            ]
            ...
            if not parent_id or not content:
                raise ValueError("parent_id and content are required")
                
            # Convert plain text content to rich text format
            rich_text = [
                {
                    "type": "text",
                    "text": {
            ...
            comment = await notion_client.create_comment(
                parent_id=parent_id,
                rich_text=rich_text
            )
            return [
                TextContent(
                    type="text",
                    text=comment.model_dump_json(indent=2)
                )
            ]
            
        elif name == "get_comments_with_context":
            if not isinstance(arguments, dict):
                raise ValueError("Invalid arguments")
            ...
            # Add context information to each comment
            for comment in comments.results:
                comment_data = comment.model_dump()
                
                if include_context:
                    try:
                        context = await notion_client.get_comment_context(comment)
                        comment_data["context"] = context
                    except Exception as e:
                        logger.warning(f"Could not get context for comment {comment.id}: {str(e)}")
                        comment_data["context"] = {"error": str(e)}
                
                result["results"].append(comment_data)
